                ...
                if (d[j].empty == false) { 
                        ...
                        if((*(groups + 0)+*(groups + 1)+*(groups + 2)+*(groups + 3))>0){ //an yparxei to disease sto country/date ayto
                               ...
                               // packets[2]=d[j].text;
                                ...
                                packet3 = "Age Range 60+ years: " + to_string(*(groups + 3)) + " cases ";
                                cout<<packet3<<endl;
                                packet4 = packet3 + packet4;
...
void StatsData::insertNode(string * s) //eisagwgi neou komvou sto telos
{
...
void StatsData::printList() { //ektypwsi listas
    StatNode* current = head;
    while (current != NULL) {
        cout << (current->line[0])<<endl;
        current = current->next;
    }
}